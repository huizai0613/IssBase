...
import com.iss.R;
import com.iss.view.tablefixheaders.adapters.TableAdapter;
...
public class TableFixHeaders extends ViewGroup {
	...
	private boolean isScrollOnlyXY = true;
	...
	boolean isLeftRight = true;// 判断左右移动方向
	boolean isTopBottom = true;// 判断上下移动方向
	boolean isDown = false;// 判断是否按下
	...
	private void shadowsVisibility() {
		...
		final int[] remainPixels = { actualScrollX, actualScrollY, getMaxScrollX() - actualScrollX, getMaxScrollY() - actualScrollY, };
	...
	private class Flinger implements Runnable {
		...
		void start(int initX, int initY, int initialVelocityX, int initialVelocityY, int maxX, int maxY) {
			// if(Math.abs(initialVelocityX-initX)>Math.abs(initialVelocityY-initY)){
			// scroller.fling(initX, initY, initialVelocityX, initY, 0, maxX, 0,
			// maxY);
			// }else{
			// scroller.fling(initX, initY, initX, initialVelocityY, 0, maxX, 0,
			// maxY);
			// }
		...
		public void run() {
			...
			if (diffX != 0 || diffY != 0) {
				if(isScrollOnlyXY){
					...
					scrollBy(diffX, diffY);//全屏任意滚动 需要打开这个
				}
				lastX = x;
				lastY = y;
			}
	...
	/**
	 * 设置是否是xy轴滚动，true为x，y周滚动，false为任意滚动，默认是true
	 * @param isScrollOnlyXY
	 */
	public void setScrollOnlyXY(boolean isScrollOnlyXY){
		this.isScrollOnlyXY = isScrollOnlyXY;
	}
