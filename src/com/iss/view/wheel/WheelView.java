...
public class WheelView extends View {
		...
		if (centerDrawable == null) {
			...
			if(isFirst){
				...
				centerDrawable = getContext().getResources().getDrawable(
						R.drawable.wheel_select_province);
			}else{
				setBackgroundResource(R.drawable.wheel_bg2);
				centerDrawable = getContext().getResources().getDrawable(
						R.drawable.wheel_select_city);
	...
	private void drawItems(Canvas canvas) {
		...
		if (scaleItem) {
			...
			for (int i = 0; i < count; i++) {
				...
				float scale = (visibleItems - Math.abs(i - currentItem
						+ firstItem))
						/ (1f * visibleItems);
				...
				//
				// int centerHeight = (int)view.get- height / 2;
				//
			}
		}
		...
		for (int i = 0; i < count; i++) {
			...
			if (i == currentItem - firstItem) {
				...
				if (tag instanceof TextView) {
					((TextView) tag).setTextColor(getResources().getColor(
							R.color.wheel_select_text_color));
				}
				...
				if (tag instanceof TextView) {
					((TextView) tag).setTextColor(getResources().getColor(
							R.color.wheel_unselect_text_color));
				}
			}
		}
	...
	private void drawCenterRect(Canvas canvas) {
		centerRectcenter = getHeight() / 2;
		int offset = (int) (getItemHeight() / 2 * 1.2);
		centerDrawable.setBounds(0, centerRectcenter - offset, getWidth(),
				centerRectcenter + offset);
